...
test("admin can orchestrate control assurance and review it in the manager dashboard", async ({
  ...
  await Promise.all([
    page.waitForURL(/\/dashboard\/controls\/[0-9a-f-]+/, {
      timeout: 45_000,
    }),
    page.getByRole("button", { name: "Create control" }).click(),
  ]);
  ...
  await attestationSection
    .getByRole("button", { name: "Launch attestation cycle" })
    .click();

  await expect(page).toHaveURL(/success=attestation_created/, {
    timeout: 45_000,
  });
  ...
  const attestationUpdateForm = attestationSection
    .locator("form")
    .filter({
  ...
  await attestationUpdateForm
    .getByRole("button", { name: "Update attestation" })
    .click();

  await expect(page).toHaveURL(/success=attestation_updated/, {
    timeout: 45_000,
  });
  ...
  await requestsSection
    .getByRole("button", { name: "Add evidence request" })
    .click();

  await expect(page).toHaveURL(/success=evidence_request_created/, {
    timeout: 45_000,
  });
  ...
  await requestsSection
    .getByRole("link", { name: "Upload evidence" })
    .first()
    .click();
  await expect(page).toHaveURL(/\/dashboard\/evidence\/new/, {
    timeout: 45_000,
  });
  await expect(
  ...
  await page
    .getByLabel("Description")
    .fill("Uploaded from the control assurance evidence request.");
  ...
  await expect(page).toHaveURL(/success=evidence_uploaded/, {
    timeout: 45_000,
  });
  await expect(
    page.getByText("Evidence uploaded and linked to the request."),
  ).toBeVisible({
    timeout: 20_000,
  });

  const requestUpdateForm = requestsSection
    .locator("form")
    .filter({
  ...
  await requestUpdateForm
    .getByRole("button", { name: "Update request" })
    .click();

  await expect(page).toHaveURL(/success=evidence_request_updated/, {
    timeout: 45_000,
  });
  ...
  await expect(
    page.getByRole("heading", { name: "Control assurance" }),
  ).toBeVisible();
  const controlRow = page.locator("tr").filter({
    has: page.getByRole("link", { name: `${controlCode} - ${controlTitle}` }),
  });
  await expect(
    controlRow.getByRole("link", { name: `${controlCode} - ${controlTitle}` }),
  ).toBeVisible();
